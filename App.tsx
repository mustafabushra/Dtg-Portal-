...
const App: React.FC = () => {
  ...
  const [themeSettings, setThemeSettings] = useState(() => loadData('cafe_theme', {
    systemName: 'كافي برو',
    primaryColor: '#f59e0b',
    logoUrl: 'https://placehold.co/200x200?text=CAFE'
  }));
  ...
  useEffect(() => {
    ...
    const r = parseInt(themeSettings.primaryColor.slice(1, 3), 16);
    const g = parseInt(themeSettings.primaryColor.slice(3, 5), 16);
    const b = parseInt(themeSettings.primaryColor.slice(5, 7), 16);
  ...
  const handleLogin = (type: 'ADMIN' | 'STAFF', staffId?: string, password?: string) => {
    if (type === 'ADMIN') {
      ...
      if (user && user.password === password) {
        ...
        alert('رمز الدخول خاطئ أو الموظف غير موجود');
      }
    }
  };
  ...
  return (
    <div className="min-h-screen flex bg-[#f8fafc] font-['Cairo'] text-slate-900 overflow-x-hidden">
      <style>{`
        ...
        .bg-custom-light { background-color: var(--primary-color-light) !important; }
        .hover-bg-custom-primary:hover { background-color: var(--primary-color) !important; }
        .shadow-custom { shadow-color: var(--primary-color) !important; }
      `}</style>
      ...
      <main className={`flex-1 transition-all duration-300 w-full lg:mr-64 p-4 md:p-8 lg:p-10`}>
        <div className="mb-6 md:mb-8 flex items-center justify-between bg-white px-4 md:px-8 py-3 md:py-5 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-200">
          <div className="flex items-center gap-2 md:gap-6">
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-slate-500">مزامنة سحابية نشطة</span>
            </div>
          </div>
